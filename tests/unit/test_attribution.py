...
def labelled(**kw: object) -> Attribution:
    base = dict(
        address=BINANCE_14, chain=ETHEREUM, label="Binance 14",
        category=Category.CEX, confidence=Confidence.HIGH,
        method=Method.LABEL, source="etherscan-nametag@2026-08",
    )
...
class TestConstruction:
    ...
    def test_weak_claims_must_justify_themselves(self):
        ...
        with pytest.raises(ValueError, match="requires a rationale"):
            Attribution(
                address=UNLABELLED_BTC, chain=BITCOIN, label="Some exchange",
                category=Category.CEX, confidence=Confidence.LOW,
                method=Method.INFERENCE, source="analyst",
            )
...
class TestConfidence:
    ...
    def test_weak_claims_are_marked_in_display(self):
        weak = Attribution(
            address=UNLABELLED_BTC, chain=BITCOIN, label="Some exchange",
            category=Category.CEX, confidence=Confidence.LOW,
            method=Method.INFERENCE, source="analyst", rationale="fee-rate match",
        )
...
class TestMerge:
    ...
    def test_sanctions_take_the_primary_slot(self):
        ...
        friendly = Attribution(
            address=TORNADO_100, chain=ETHEREUM, label="Tornado.Cash: 100 ETH",
            category=Category.MIXER, confidence=Confidence.HIGH,
            method=Method.LABEL, source="etherscan-nametag",
        )
        sanctioned = Attribution(
            address=TORNADO_100, chain=ETHEREUM, label="OFAC SDN",
            category=Category.SANCTIONED, confidence=Confidence.CERTAIN,
            method=Method.LIST, source="ofac-sdn@2026-08-01",
        )
    ...
    def test_sanctions_win_even_at_lower_confidence(self):
        strong = labelled(
            address=RONIN_EXPLOITER, confidence=Confidence.CERTAIN,
            category=Category.SUSPECT, label="Ronin Bridge Exploiter",
        )
        weak_sanction = Attribution(
            address=RONIN_EXPLOITER, chain=ETHEREUM, label="possible SDN match",
            category=Category.SANCTIONED, confidence=Confidence.LOW,
            method=Method.INFERENCE, source="analyst",
    ...
    def test_higher_confidence_wins_otherwise(self):
        weak = Attribution(
            address=BINANCE_14, chain=ETHEREUM, label="unknown service",
            category=Category.SERVICE, confidence=Confidence.MEDIUM,
            method=Method.HEURISTIC, source="consolidation-analysis",
    ...
    def test_recency_breaks_remaining_ties(self):
        old = labelled(label="Old Name",
                       observed_at=datetime(2024, 1, 1, tzinfo=UTC))
        new = labelled(label="New Name",
    ...
    def test_weak_claims_do_not_create_a_dispute(self):
        e = merge([
            ...
            Attribution(
                address=BINANCE_14, chain=ETHEREUM, label="maybe a mixer",
                category=Category.MIXER, confidence=Confidence.LOW,
                method=Method.INFERENCE, source="analyst", rationale="hunch",
            ),
        ])
    ...
    def test_terminal_if_any_claim_is_terminal(self):
        e = merge([
            labelled(category=Category.SUSPECT, confidence=Confidence.MEDIUM,
                     method=Method.HEURISTIC, rationale="high deposit fan-in"),
